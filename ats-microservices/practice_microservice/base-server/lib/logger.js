...
logger.setupFileLogger = function setupFileLogger() {
    ...
    try {
        ...
        if (process.env.NODE_ENV !== "test") {
            logger.error(chalk.red("An error has occured during the creation of the File transport logger."));
...
logger.getLogOptions = function getLogOptions() {
    ...
    if (!_.has(_config, "log.fileLogger.directoryPath") || !_.has(_config, "log.fileLogger.fileName")) {
        logger.error("unable to find logging file configuration");
        return false;
    }
...
logger.getLogFormat = function getLogFormat() {
    ...
    if (!_.includes(validFormats, format)) {
        ...
        if (process.env.NODE_ENV !== "test") {
            logger.error(chalk.yellow("Warning: An invalid format was provided. The logger will use the default format of \"" + format + "\""));
        }
    }
